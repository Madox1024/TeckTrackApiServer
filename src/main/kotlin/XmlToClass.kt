...
fun getRaceDataMap(doc: Document): MutableMap<String, String> {
...
// Takes MutableMap from getRaceData and returns a com.tecktrack.leaderboard.RaceData Obj, IllegalArgumentException if null values given
fun raceDataMapToClass(xmlMap: MutableMap<String, String>): RaceData {
    try {
        return RaceData(
...
//  Takes xml doc and returns a mutable map of [regnum : com.tecktrack.leaderboard.CarData] (this fun feels like a monster D:)
fun getCarDataMap(doc: Document): MutableMap<String, CarData> {
    val xpFactory = XPathFactory.newInstance()
    val xPath = xpFactory.newXPath()

    val elementXPath = "/resultspage/results/result"
    val labelNodeList = xPath.evaluate(elementXPath, doc, NODESET) as NodeList
    val carMap: MutableMap <String, CarData> = mutableMapOf()
    ...
    for (i in 0 until labelNodeList.length) { // Node lists aren't iterable (thanks .js)
        try {
            val carData = CarData(
                    ...
                    bestLapTime = parseTimestamp(labelNodeList.getAttributeValue(i, "besttime")
                    ).toMilliseconds(),
                    lastLapTime = parseTimestamp(labelNodeList.getAttributeValue(i, "lasttime")
                    ).toMilliseconds(),
